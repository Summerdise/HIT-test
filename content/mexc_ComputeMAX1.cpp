 ...
 *      mexc_ComputeMAX1( nGaborOri, S1Map, locationPerturbationFraction, 
 *      orientationPerturbationRadius, sizeGaborPerScale, subsampleM1 );
 *
 */
...
int orientationPerturbRadius;               /* allowed orientation perturbation */
float locationPerturbRadius;                /* allowed location perturbation, relative to the Gabor filter size */
const int numLocationShiftPerSide = 2;
...
void StoreShift()
{
    ...
    //numShift = (int*)mxCalloc( nGaborScale, sizeof(*numShift) );
    numShift = (numLocationShiftPerSide * 2 + 1) * (orientationPerturbRadius*2+1);
    ...
    for( iS = 0; iS < nGaborScale; ++iS ) /* for each scale */
    {
        for (orient=0; orient<nGaborOri; orient++)  /* for each orientation */
        {
            ...
            ci = 0;
            for( i=0; i<=numLocationShiftPerSide; ++i ) // location shifts
            {
                for (si=0; si<=1; si++) /* sign of i */
                {
                    cj = 0;
                    for( j = 0; j<=orientationPerturbRadius; j++ ) // orientation shifts
                    {
                        for (sj=0; sj<=1; sj++) /* sign of j */
                        {
                            shift = ci*(2*orientationPerturbRadius+1) + cj; /* hybrid counter for location and orientation */
                            
                            double location_shift = (double)i / (double)numLocationShiftPerSide
                                * (double)sizeGabor[iS] * (double)locationPerturbRadius;
                            
                            // mexPrintf("locationshift=%.3f, pr = %.3f, sizegabor=%.3f\n", location_shift, locationPerturbRadius,sizeGabor[iS]);
                 ...
                 int nrhs, const mxArray *prhs[])                
{
     ...
     */
    orientationPerturbRadius = (int)mxGetScalar(prhs[3]);
    
    /*
     * input variable 4: sizes of Gabor filters
     */
    sizeGabor = (float*)mxGetPr(prhs[4]);
    
    /*
     * input variable 5: sub sample step length
     */
    subsample = (int)mxGetScalar(prhs[5]);
